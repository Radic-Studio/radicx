...
select ok((select relrowsecurity from pg_class where oid='private.question_keys'::regclass), 'private question keys RLS enabled');
select ok((select relrowsecurity from pg_class where oid='private.staff_roles'::regclass), 'private staff roles RLS enabled');
select throws_ok(
